...
public class BTree {
    ...
    public int getLastId(long pageStart) {
        try {
            int lastId = 0;
            RandomAccessFile table = new RandomAccessFile(tableName, "rw");
            table.seek(pageStart + Davisbase.getPageSize());
            table.readByte();
            table.readByte();
        ...
        if (pagePointer != 0L) {
            try {
                ...
                if (OverflowBucket != 0L) {
                    long newPageStart = Davisbase.getPageSize() * (table.length() / Davisbase.getPageSize() + 2L);
                    ...
                    table.seek(pagePointer + Davisbase.getPageSize());
                    table.readByte();
                    table.readInt();
                    table.readInt();
                    table.writeInt(0);
    ...
    public void checkOverflow() {
        try {
            RandomAccessFile table = new RandomAccessFile(tableName, "rw");
            long newPageStart = Davisbase.getPageSize() * (table.length() / Davisbase.getPageSize() + 2L);
            if (table.length() > Davisbase.getPageSize() * 3L) {
                long pagePointer = -1L;
                while (table.getFilePointer() < table.length()) {
                    ...
                    table.seek(table.getFilePointer() + Davisbase.getPageSize() - 1L);
                }
                ...
                if (pagePointer == -1L) {
                    ...
                    table.writeInt((int) (leftPointer + Davisbase.getPageSize() * 3L));
                    table.writeInt(0);
                    table.writeInt(0);
                    table.close();
                } else {
                    ...
                    if (overflowBucket != -1L) {
                        ...
                        table.seek(lastLeafPagePointer + Davisbase.getPageSize());
                        table.readByte();
                        table.readInt();
                        table.readInt();
                        table.writeInt(0);
    ...
    public long insert(int recordSize) {
        try {
            ...
            long pageStart = 0L;
            long pageEnd = pageStart + Davisbase.getPageSize() - 1L;
            if (filelength == 0L) {
                table.close();
            ...
            if ((pageType == 1) && (rightPointer == 0)) {
                ...
                pageEnd = pageStart + Davisbase.getPageSize();
                cells++;
                if (startPointer - recordSize > pageStart + 8L + 2 * cells) {
                    updateLeafHeader(pageStart, pageEnd, recordSize, rightPointer);
                } else {
                    rightPointer = (int) ((filelength + 1L) / Davisbase.getPageSize() * Davisbase.getPageSize());
                    updateRightPointerOfLeafHeader(pageStart, pageEnd, recordSize, rightPointer);
                    writeLeafHeader(rightPointer, rightPointer + Davisbase.getPageSize() - 1L, recordSize, 0);
                    checkOverflow();
                }
            }
            table.close();
